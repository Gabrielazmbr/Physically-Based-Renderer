...
Implementation notes for the custom Principled BSDF. The detailed derivation below follows the opaque reflection branch in code-reading order. It began as a development learning document, so short snippets are explanatory rather than a substitute for the current implementation in `principled.py`.

The current plugin extends that base branch with textured parameters, anisotropic GGX, optional Burley diffuse, GTR1 clearcoat, evaluated sheen, and smooth thin-sheet or solid dielectric transmission. Formal evidence is documented in [`tests/evaluation/bsdf`](../tests/evaluation/bsdf/README.md) and [`tests/evaluation/features`](../tests/evaluation/features/README.md).
...
```text
              ...
              ---------------------------------
              |                               |
       Opaque reflection              Smooth transmission
              |                      thin sheet / solid glass
      --------------------
      |        |         |
   diffuse    GGX      clearcoat
 Lambert/Burley      + evaluated sheen
```

`sample()` chooses a transport event, `eval()` evaluates non-delta
reflection, and `pdf()` reports the matching continuous density.

Three functions, three jobs. `sample()` is called by the path tracer's
BSDF-sampling step (see [`integrators/README.md`](../integrators/README.md), section 8) to pick a
...
The opaque base branch is a mixture of two building blocks: a diffuse lobe and a glossy GGX lobe, with `metallic` controlling their balance. Clearcoat and sheen extend that branch, while `transmission` selects a separate smooth-dielectric branch.
...
These are the three core base-mixture parameters. The current constructor also
reads specular, anisotropy, clearcoat, sheen, diffuse-model, transmission, IOR,
thin-sheet, and texture controls.
...
`m_flags` and `m_components` begin with diffuse and glossy reflection. At construction time the plugin additionally advertises delta reflection/transmission and back-side support when transmission is enabled, and appends a separate glossy component when clearcoat is enabled. Component indices are therefore derived by the implementation rather than assumed globally.
...
def _spec_prob(self, metallic, specular):
    base_prob = dr.select(metallic > 0.99, 1.0,
        dr.clamp(metallic * 0.8 + 0.1, 0.1, 0.9))
    return dr.select((specular > 0) | (metallic > 0), base_prob, 0.0)
```

Within the opaque branch, this is a number between 0 and 1: the odds of a **weighted coin flip** used to choose the base GGX lobe rather than diffuse. A zero-specular, zero-metallic material now returns exactly zero; clearcoat and transmission are handled by outer mixture choices. "Weighted" doesn't mean 50/50; it means *unequal* odds,
...
weighting on the *next* bounce ([`integrators/README.md`](../integrators/README.md), section 6).
...
[`integrators/README.md`](../integrators/README.md)), how much light actually reflects that way?"
...
called directly by NEE ([`integrators/README.md`](../integrators/README.md), section 7) to weight
