...
Implementation notes for the custom path-tracing integrator, organised in code-reading order. This began as a development learning document, so snippets explain the algorithm and may omit newer options; `path_tracer.py` remains authoritative.

The current integrator includes NEE/MIS, Russian roulette, path-depth controls, optional AOVs, per-contribution firefly clamping, camera-visibility controls, and an opt-in straight-through transparent-shadow approximation. Formal evidence is in [`tests/evaluation/path_tracer`](../tests/evaluation/path_tracer/README.md), [`tests/evaluation/sampling`](../tests/evaluation/sampling/README.md), and [`tests/evaluation/features`](../tests/evaluation/features/README.md).
...
Scattering Distribution Function), which describes how much the material reflects light
...
- **`max_depth`** — the maximum number of bounces allowed for a ray before
  forced termination.
- **`rr_depth`** — the bounce number at which Russian roulette begins being considered (section 9).
- **`with_aovs`** — returns primary-hit albedo, world-space shading normal, and depth alongside beauty.
- **`firefly_clamp`** — optional biased cap on individual contributions; zero disables it.
- **`transparent_shadows`** and **`max_transparent_shadow_depth`** — opt-in straight-through shadow visibility for smooth transmissive panes.
- **`hide_from_camera`** and **`opaque_shadow_shapes`** — scene-integration visibility controls identified by configured IDs.
...
- **valid_ray** — whether the path contributed at all, used by the film
  for alpha/coverage.
- **AOVs (Arbitrary Output Variables)** — when `with_aovs=True`, the integrator returns primary-hit albedo RGB, world-space shading-normal XYZ, and depth after beauty RGB. Miss channels remain zero. With AOVs disabled, the list is empty.
